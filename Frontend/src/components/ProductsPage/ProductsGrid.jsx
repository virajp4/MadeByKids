...
export default function ProductsGrid({ limit = 2, showCart = true, products, showSearch = true }) {
  return (
    <div className="">
      {showSearch && (
      ...
      <div className="p-4 sm:px-5 sm:py-5 lg:px-8">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-4 gap-x-6 gap-y-10 xl:gap-x-8 ">
          {products.map((product, idx) => (
            <ProductGridItem key={idx} product={product} showCart={showCart} />
          ))}
        </div>
      </div>
    </div>
